...
Send trigger values to the parallel port.
- Supports both Desktop's native LPT and USB2LPT.
- Supports software trigger
- Use MockTrigger class for testing purposes.
...
int init(duration)
	Returns -1 if error, duration if success.
	Duration unit: msec

void signal(value)
	Sends the value to the parallel port and sets to 0 after a set period.
	The value is an integer in the range of 0-255.
...
class Trigger:
	def __init__(self, lpttype='USB2LPT', portaddr=None):
		self.evefile= None
		...
		if self.lpttype in ['USB2LPT', 'DESKTOP']:
			...
			for f in search:
				if os.path.exists(f):
					dllpath= f
					break
				else:   # modified, one more indent_Dong
...
if __name__=='__main__':
	...
	# Arduino
#	trigger= Trigger('ARDUINO', None) #ARDUINO, None
