...
import ConfigParser
import sys, os, time, math, string, exceptions
import BCI
import struct
...
###################################################################################
if len(sys.argv) == 1:
	SERIAL= raw_input('Serial number? Just hit enter to use any EEG amp.\n>> ').strip()
	if SERIAL=='':
		SERIAL= None
else:
	SERIAL= sys.argv[1]
	print('Using the amp with serial %s'% SERIAL)
###################################################################################
...
KNOWN_SERVERS= ['gUSBamp','BioSemi','StreamPlayer']
...
class Scope(QtGui.QMainWindow, form_class):
	...
	def init_scope_GUI(self):
		...
		for x in range(0, self.config['eeg_channels']):
			if (self.show_channel_names):
				self.channel_labels.append("(" + str(x+1) + ") " + self.scope_settings.get("internal", "channel_names_" + self.device_name + str(self.config['eeg_channels'])).split(', ')[x])
			else:
		...
		self.main_plot_handler.setRange(xRange=[0, self.seconds_to_show], yRange=[+1.5*self.scale, -0.5*self.scale-self.scale*self.config['eeg_channels']])
		...
		self.curve_eeg = []
		for x in range (0, len(self.channels_to_show_idx)):
			self.curve_eeg.append(self.main_plot_handler.plot(x=self.x_ticks, y=self.data_plot[:,self.channels_to_show_idx[x]], pen=pg.mkColor(self.colors[self.channels_to_show_idx[x]%16,:])))
	...
	def init_loop(self):
		...
		amp_trigger= None # for separate trigger server
		amps= []
		server_found= False
		...
		while server_found == False:
			...
			if len(streamInfos) == 0:
				print('Waiting for LSL server on the network...')
				time.sleep(1)
				continue

			for si in streamInfos:
				if si.type()=='Markers':
					print('>> Found marker server %s @ %s.' % (si.name(), si.hostname()) )
					amp_trigger= si

				inlet= pylsl.StreamInlet(si)
				amp_serial= inlet.info().desc().child('acquisition').child_value('serial_number')

				if SERIAL is not None and SERIAL != amp_serial:
					continue

				if 'USBamp' in si.name():
					tr_channel= 16
					amps.append(si)
					server_found= True
					print('>> Found EEG server %s @ %s.' % (si.name(), si.hostname()) )
					break
				elif 'BioSemi' in si.name():
					tr_channel= 0 # or subtract -6684927? (value when trigger==0)
					amps.append(si)
					server_found= True
					print('>> Found EEG server %s @ %s.' % (si.name(), si.hostname()) )
					break
				elif 'StreamPlayer' in si.name():
					tr_channel= 0
					amps.append(si)
					server_found= True
					print('>> Found EEG server %s @ %s.' % (si.name(), si.hostname()) )
					...
					print('Skipping unrecognized server', streamInfos[0].name())

		# create new inlets to read from the stream
		self.inlets= []
		for amp in amps:
			self.inlets.append( pylsl.StreamInlet(amp) )
		if amp_trigger is not None:
			self.inlet_trigger= pylsl.StreamInlet(amp_trigger)
		else:
			self.inlet_trigger= None

		self.num_amps= len(amps)
		#eeg_channels= eeg_ch[1] - eeg_ch[0]
		srate= int(amps[0].nominal_srate())
		n_channels= amps[0].channel_count()
		print('Number of channels: %d' % n_channels )
		print('LSL Protocol version: %s'% amps[0].version() )
		print('Source sampling rate: %.1f\n'% srate )

		# 12 unsigned ints (4 bytes)
		data= ['EEG', srate, ['L','R'], 32, 16, 0, 0, None, None, None, None, None]
	...
	def update_loop(self):
		try:
			...
			#self.updating= False
			#print('$$$$', self.tm.sec(), len( self.ts_list ))
			#self.tm.reset()
	...
	def read_eeg(self):
		...
		try:
			data, self.ts_list= self.inlets[0].pull_chunk(max_samples=self.config['sf']) # [frames][channels]
			if len(self.ts_list) == 0:
				self.eeg= None
				self.tri= None
				return
			
			# assume trigger channel is the last channel for USBamp
			if self.inlet_trigger is None:
				n= self.config['eeg_channels']
				x= np.array( data )
				self.eeg= np.reshape( x[:,:n], (-1,n) ) # samples x channels
				self.tri= np.reshape( x[:,-1], (-1,1) ) # samples x 1
			# when there's a separate trigger server
			else:
				# Assume the number of samples are identical for both eeg and trigger inlets
				btri, self.ts_list_tri= self.inlet_trigger.pull_chunk(max_samples=self.config['sf']) # [frames][channels]
				self.eeg= np.reshape( data, (len(data), self.config['eeg_channels']) )
				self.tri= np.reshape( btri, (len(btri), 1) )
			
			if False:
				# show trigger value
				try:
					print( 'Received trigger',  max(self.tri) )
				except:
					print( 'Error! self.tri =', self.tri )
			...
			print('**** Unexpected error in read_eeg():\n%s\n%s'% (sys.exc_info()[0], sys.exc_info()[1]))
			sys.exit(-1)
	...
	def update_ringbuffers(self):
		...
		if (self.show_LPT_events) and (not self.stop_plot):
			...
			for x in range (1, len(self.tri)):
				if (self.tri[x] != self.tri[x-1]) and self.tri[x] != 0:
					self.addEventPlot("LPT", int(self.tri[x]) )
	...
	def paintEvent(self, e):
		...
		if (sender == None) and (not self.force_repaint):
			pass
		else:
			self.force_repaint = 0
			qp = QtGui.QPainter()
	...
	def paintInterface(self, qp):
		...
		for x in xrange (0, len(self.events_detected), 2):
			xh= int(x/2)
			self.events_curves[xh].setData(x=np.array([self.x_ticks[self.events_detected[x]], self.x_ticks[self.events_detected[x]]]),\
	...
	def addEventPlot(self, event_name, event_id):
		...
		self.events_curves.append(self.main_plot_handler.plot(pen=color, x=np.array([self.x_ticks[-1], self.x_ticks[-1]]), y=np.array([+1.5*self.scale, -1.5*self.scale*self.config['eeg_channels']])))
	...
	def onSelectionChanged_table(self):
		...
		self.curve_eeg = []
		for x in range (0, len(self.channels_to_show_idx)):
			self.curve_eeg.append(self.main_plot_handler.plot(x=self.x_ticks, y=self.data_plot[:,self.channels_to_show_idx[x]], pen=self.colors[self.channels_to_show_idx[x]%16,:]))
			self.curve_eeg[-1].setDownsampling(ds=self.subsampling_value, auto=False, method="mean")
